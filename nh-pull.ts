#!/usr/bin/env -S deno run --allow-net=hacker-news.firebaseio.com --allow-read --location https://wiremoons.com/
//
//  Run from the command line with Deno in path as command:
//  deno run --allow-net=hacker-news.firebaseio.com ./hn-pull.ts
//
// Application TODO:
// - add `localStorage` timesamp so can check elapsed time since last run
